import bs4, webbrowser, sys, requests, os, math, json
#https://www.rwwa.com.au/cris/raceresults.aspx?meeting=5154427
...
class MapIt_Bs_Starters_Results(object):
    def __init__(self, Mode, Link):
        ...
        self.RaceNumber = self.GetRaceNumber()
        self.HasRaceNumber = False
        #print("Here is the race number: " + str(self.RaceNumber))
    ...
    def GetRaceNumber(self):
        index = self.WebAddress.find("race=")
        if index == -1:
            self.HasRaceNumber = False
            return None
        else:
            index += 5
        return self.WebAddress[index:]
    ...
    def RequestLink(self):
        ...
        #link = "https://www.rwwa.com.au/cris/raceresults.aspx?meeting=5154427"
    ...
    def MeetingHasNextRaceOdds(self, index):
        ...
        if self.HasRaceNumber == False:
            ...
            newIndex = index+1
            newRequestLink = self.WebAddressField
            response = requests.get(newRequestLink)
            #print(newRequestLink)
            if (response.status_code == 404):
                return False
            ...
            tables = racenoSoup.find_all("table", {"class": "tblcris raceField"})
            self.LocalCycleData = bs4.BeautifulSoup(str(tables[0]), "html5lib")
            self.PageData = racenoSoup
            if len(tables) == 0:
                return False
            return True
    ...
    def CheckRaceNum(self):
        if self.RaceNumber == None:
            self.HasRaceNumber = False
        else:
            self.HasRaceNumber = True
    ...
    def MeetingHasNextRace(self, index):
        ...
        if self.HasRaceNumber == False:
            ...
            newIndex = index+1
            newRequestLink = self.WebAddress
            response = requests.get(newRequestLink)
            #print(newRequestLink)
            if (response.status_code == 404):
                return False
